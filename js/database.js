...
let grammarDb = []; // Globálna premenná pre vety
    ...
    try {
        ...
        if (wb.SheetNames[1]) {
            ...
            grammarDb = gRows.map((r, i) => ({
                ...
                lekcia: parseInt(r['Lekcia']) || 0,
                sk: r['Slovenský'],
                ja: r['Japonský'], // Celá veta v japončine (kana/kanji)
                romaji: r['Rómadži'] // Veta v romaji
            })).filter(g => g.sk && g.ja && g.lekcia > 0);
        }
